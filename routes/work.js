...
//根据时间获取最新的5部作品
router.get('/getRecommendedWorks',function(req,res,next){
    let userId = req.param('userId');  //暂时不用，现在根据时间，获取最新的5篇
        ...
        .then(function(docs){
            if(userId){
                User.findEverythings({_id:userId}).then(function(userDocs){
                    var favouriteWorkList = userDocs.favouriteWorkList;
                    if(favouriteWorkList!=null){
                        var praiseWorkList = userDocs.praiseWorkList;
                        docs.forEach(item=>{
                            favouriteWorkList.forEach(item1=>{
                                if(item._id==item1){
                                    docs.liked = true;
                                }
                            });
                            praiseWorkList.forEach(item2=>{
                                if(item._id==item2){
                                    docs.praised = true;
                                }
                            });
